...
export const getProductsForSubscriptionFromProductStockByStoreId = async id => {
  return axiosApiInstance.get(`${serverUrl}/getProductsForSubscriptionFromProductStockByStoreId/${id}`);
};

export const getAllProductByUser = async query => {
  console.log(`${serverUrl}/getAllStoreOrderProduct/?limit=5&${query}`, 'ADASDASD');
  return axiosApiInstance.get(`${serverUrl2}/getAllStoreOrderProduct/?limit=5&${query}`);
};
